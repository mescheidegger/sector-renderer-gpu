...
`dynamicSectorIds` contains exact sector IDs whose floor and dynamic seam presentation must not be baked into the static mesh because the caller intends to submit moving or replacement presentation through `worldQuads`. Owned walls and walls directly portal-linked to a dynamic sector are delegated; ceilings follow the normal sector/parent rules. Example uses include an animated platform, a deforming room component, or a surface generated by an external simulation.
...
Static mesh groups are separated by material, projection, and surface type so walls retain double-sided/no-cull presentation while planar floors and ceilings remain one-sided. For `dynamicSectorIds`, floors and owned walls are omitted, and walls directly connected to a dynamic ID through `portalTo` or `portalLinks` are also delegated. This lets callers submit one authoritative dynamic presentation for both physical sides of a moving seam.
