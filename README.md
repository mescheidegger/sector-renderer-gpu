...
```js
{
  ...
  portalLinks?: Array<{ sectorId: string | number, bottomZ: number, topZ: number }>,
  uvScale?: number
}
```
...
- `portalLinks` declares exact vertically bounded neighbors and is mutually exclusive with non-null `portalTo`.
...
Static draw grouping distinguishes material, projection mode, and surface type. One texture key may safely appear on world- and sky-projected surfaces, while double-sided/no-cull walls remain separate from one-sided, back-face-culled floors and ceilings.
...
`dynamicSectorIds` contains exact sector IDs whose floor heights the caller controls. Use `createDynamicSectorWorldQuads(world)` with the same world passed to `SectorRenderer` to prepare their replacement presentation:

```js
import { createDynamicSectorWorldQuads } from 'sector-renderer-gpu';

const buildDynamicQuads = createDynamicSectorWorldQuads(world);
// Each update supplies absolute floor Z values, keyed by exact dynamic sector ID.
const floorZBySectorId = new Map([['platform', currentFloorZ]]);
renderer.render({ camera, timeSeconds, worldQuads: buildDynamicQuads(floorZBySectorId) });
```

The builder uses the same surface triangulation, seam resolution, material ownership, UV mapping, lighting, deduplication, and corner stitching as the static mesh. It replaces all dependent seam surfaces, including bands owned by static neighbors, and preserves nested-floor suppression. Seams whose offset/coplanar geometry can interact stay together in the replacement batch. Ceilings retain their normal static sector/parent behavior.

Floor heights must be finite and below the sector ceiling. Omitted IDs use their authored floor heights. Authored portal-link bounds remain fixed and are intersected with both sectors' current vertical extents. The caller owns movement, timing, and simulation; the builder does not mutate the world or advance motion. Recreate it when topology, materials, ceilings, dynamic IDs, or portal metadata change, alongside `renderer.replaceWorld(world)`. Returned arrays are cached for unchanged heights and should be treated as read-only. This API supports vertical floor movement, not arbitrary sector deformation.
...
`surfaceConstraints` is optional presentation metadata for keeping an upright billboard in front of vertical world surfaces as the camera angle changes. Each constraint has a horizontal `normal` pointing toward the allowed side and a horizontal `point` located on the actual surface plane. An optional nonnegative `clearanceOffset` requests spacing beyond the billboard's projected half-width, such as clearance for geometry extending in front of its reference plane. The renderer normalizes valid normals and ignores zero-length or non-finite constraints. It measures the submitted center's signed distance from each plane—thereby preserving spacing already applied by the caller—and shifts the center by the minimum additional displacement that clears every supplied surface. A plane already farther away than the projected billboard half-width requires no additional push. A sprite without active valid constraints uses its submitted position exactly as a normal billboard does. Setting `surfaceAttachment.mode` to `'preserve-screen-anchor'` moves the center toward the camera by a scale factor and uniformly applies that factor to the billboard dimensions. This camera-centered transformation leaves every vertex on its original camera ray, preserving the complete projected screen geometry while solving all surface planes as a one-dimensional feasible scale interval. If no positive scale is feasible, the submitted placement is retained. Omitting `surfaceAttachment` uses the default minimum-displacement multi-plane solver at full scale.
...
### Vertically partitioned portals

`RendererWall` may declare either `portalTo?: RendererId | null` or `portalLinks?: Array<{ sectorId: RendererId, bottomZ: number, topZ: number }>`; a non-empty `portalLinks` array is mutually exclusive with `portalTo`. Each finite positive interval must be non-overlapping on its wall, reference an existing sector, and lie inside the shared vertical extent. Solid wall bands are emitted outside the declared openings. Reciprocity is recommended but not required by the reusable renderer boundary.

Static mesh groups are separated by material, projection, and surface type so walls retain double-sided/no-cull presentation while planar floors and ceilings remain one-sided. For `dynamicSectorIds`, dependent floors and seams are omitted from the static mesh and supplied by `createDynamicSectorWorldQuads`, including reciprocal `portalTo`/`portalLinks` surfaces. This keeps both sides of each moving seam under the same renderer geometry rules.
